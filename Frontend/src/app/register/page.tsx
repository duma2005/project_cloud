...
export default function RegisterPage() {
  ...
  async function handleSubmit(event: FormEvent) {
    ...
    setLoading(true);
    const result = await register(trimmedEmail, password, fullName.trim() || undefined);
    setLoading(false);
      ...
      <div className="container">
        <div className="card max-w-xl p-6 space-y-4">
          ...
          <div className="flex items-center gap-3">
            <Link href="/" className="focus-ring inline-flex rounded-md border border-border px-4 py-2 text-sm">
              Go home
            </Link>
            <button
              type="button"
    ...
    <div className="container">
      <div className="card max-w-xl p-6 space-y-5">
        <div className="space-y-1">
          ...
          <p className="text-sm text-muted">Register with your email and password.</p>
        </div>

        <form onSubmit={handleSubmit} className="space-y-4">
          <label className="space-y-1 block">
            <div className="text-sm text-muted">Full name (optional)</div>
            <input
              type="text"
          ...
          <label className="space-y-1 block">
            <div className="text-sm text-muted">Confirm password</div>
            <input
              type="password"
              className="focus-ring w-full rounded-md border border-border bg-bg px-3 py-2"
              value={confirm}
